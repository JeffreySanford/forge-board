import { DiagnosticEvent} from '../diagnostic/diagnostic.interface.';
import { LogQueryResponse, LogResponse, LogFilter, LogEntry, LogLevelString, LogLevelEnum } from '../log/log-types';
import { User } from '../user/user.interface';
import { HealthData } from '../health/health.interface';
import { Metric } from '../metric/metric.interface';
import { HistoricalMetrics, SystemPerformanceSnapshot } from '../health/historical.metric.interface';
import { SocketResponse } from '../socket/socket.interface';
import { 
  JwtPayload, 
  AuthStats, 
  AuthDiagnosticEvent, 
...
} from '../jwt/jwt-diagnostics.service';
...
export interface ValidatedTypes {
  ...
  TokenVerificationOptions: TokenVerificationOptions;
  [key: string]: unknown;
}
...
export function isSocketResponse<T>(obj: unknown): obj is SocketResponse<T> {
  ...
  return (
    ...
    (typeof response.event === 'string' || response.event === undefined)
  );
}
...
export function validateSocketResponse<T>(obj: unknown): ValidationResult {
  ...
  if (response.status === 'error') {
    if (typeof response.message !== 'string') {
      issues.push('message must be a string when status is error');
    }
  }
  ...
  if (response.event !== undefined && typeof response.event !== 'string') {
    issues.push('event must be a string if provided');
  }
