...
import { KanbanSeedService, KanbanBoardData } from './kanban-seed.service';

@Injectable()
export class KanbanService {
  // Storage type - can be changed to reflect different backends
  private storageType: 'memory' | 'localStorage' | 'mongodb' | 'blockchain' =
    'memory';
  ...
  constructor(
    ...
    private readonly kanbanSeedService: KanbanSeedService
  ) {
    this.loadMockData();
  }
  ...
  private loadMockData(): void {
    try {
      // Use our new service to load the story boards
      const storyBoards = this.kanbanSeedService.loadForgeboardStories();
      if (storyBoards.length > 0) {
        // Convert KanbanBoardData to KanbanBoardDto
        const storyBoardDtos = storyBoards.map((board) =>
          this.convertBoardDataToDto(board)
        );
        this.boards.push(...storyBoardDtos);
        this.logger.info(
          `Loaded ${storyBoards.length} ForgeBoard story boards`,
          'KanbanService'
        );
      }

      // Use our new service to load example boards
      const exampleBoards = this.kanbanSeedService.loadExampleBoards();
      if (exampleBoards.length > 0) {
        // Convert KanbanBoardData to KanbanBoardDto
        const exampleBoardDtos = exampleBoards.map((board) =>
          this.convertBoardDataToDto(board)
        );
        this.boards.push(...exampleBoardDtos);
        this.logger.info(
          `Loaded ${exampleBoards.length} example boards`,
          'KanbanService'
        );
      }

      // Also try the legacy path (for backward compatibility)
      const planningBoardPath = path.join(
        __dirname,
        '../../mocks/kanban/mock-boards.json'
      );
      if (fs.existsSync(planningBoardPath)) {
        const planningData = JSON.parse(
          fs.readFileSync(planningBoardPath, 'utf8')
        );
        this.boards.push(...planningData);
        this.logger.info(
          `Loaded ${planningData.length} planning boards from kanban mock data`,
          'KanbanService'
        );
      }

      this.logger.info(
        `Total boards loaded: ${this.boards.length}`,
        'KanbanService',
        {
      ...
      this.logger.error(
        'Error loading mock data, using fallback',
        'KanbanService',
        {
          error:
              ...
              cards: [
                {
                  ...
                  title: 'Fallback Task',
                  description:
                    'This is fallback data - check mock file loading',
                  priority: 'medium',
                  tags: ['fallback'],
  ...
  getStorageType(): string {
    this.logger.debug(
      `Getting storage type: ${this.storageType}`,
      'KanbanService',
  ...
  // Set storage type
  setStorageType(
    type: 'memory' | 'localStorage' | 'mongodb' | 'blockchain'
  ): void {
    this.storageType = type;
    this.logger.info(`Storage type changed to: ${type}`, 'KanbanService', {
      storageType: type,
    });
  }
  ...
  toggleMockData(force: boolean): void {
    ...
    this.logger.info(`Mock data forced: ${force}`, 'KanbanService', {
      mockDataForced: force,
    });
  }
  ...
  checkStorageConnection(): boolean {
    ...
    this.logger.debug(
      `Checking storage connection for type: ${this.storageType}`,
      'KanbanService',
      { storageType: this.storageType }
    );
    return true;
  ...
  async getBoards(): Promise<KanbanBoardDto[]> {
    this.logger.info(
      `Getting boards from ${this.storageType} storage`,
      'KanbanService',
      {
        storageType: this.storageType,
    ...
    if (this.storageType === 'mongodb' || this.storageType === 'memory') {
      try {
        ...
        this.logger.info(
          `Found ${mongoBoards.length} boards in MongoDB`,
          'KanbanService',
          {
        ...
        if (mongoBoards.length > 0) {
          ...
          const boardDtos = mongoBoards.map((board) =>
            this.convertMongoToDto(board)
          );

          // Log detailed board info
          const totalCards = boardDtos.reduce(
            (total, board) =>
              total +
              (board.columns?.reduce(
                (colTotal, col) => colTotal + (col.cards?.length || 0),
                0
              ) || 0),
            0
          );
          ...
          this.logger.info(
            `Returning ${boardDtos.length} boards with ${totalCards} total cards from MongoDB`,
            'KanbanService',
            {
        ...
        this.logger.error(
          'Failed to load boards from MongoDB, falling back to mock data',
          'KanbanService',
          {
            error: error.message,
            storageType: this.storageType,
    ...
    this.logger.info(
      `Returning ${this.boards.length} in-memory boards`,
      'KanbanService',
      {
  ...
  async getBoardById(id: string): Promise<KanbanBoardDto> {
    this.logger.debug(`Looking for board with ID: ${id}`, 'KanbanService', {
      boardId: id,
    });
    ...
    if (this.storageType === 'mongodb' || this.storageType === 'memory') {
      try {
        const mongoBoard = await this.kanbanBoardModel
          .findOne({
            $or: [{ id }, { _id: id }],
          })
          .exec();
        ...
        if (mongoBoard) {
          this.logger.debug(
            `Found board in MongoDB: ${mongoBoard.name}`,
            'KanbanService',
            {
              boardId: id,
        ...
        this.logger.error(
          `Error querying MongoDB for board ${id}`,
          'KanbanService',
          {
            error: error.message,
            boardId: id,
    ...
    if (!board) {
      this.logger.warning(`Board with ID "${id}" not found`, 'KanbanService', {
        boardId: id,
      });
  ...
  async createBoard(createBoardDto: CreateBoardDto): Promise<KanbanBoardDto> {
    this.logger.info(
      `Creating new board: ${createBoardDto.name}`,
      'KanbanService',
      {
  ...
  async addColumn(
    ...
    this.logger.info(
      `Column ${newColumn.name} added with ID ${newColumn.id}`,
      'KanbanService',
      {
        boardId,
  ...
  async addCard(
    ...
    this.logger.info(
      `Adding card to column ${columnId} in board ${boardId}`,
      'KanbanService',
      {
        boardId,
        columnId,
    ...
    this.logger.info(
      `Card "${newCard.title}" added with ID ${newCard.id}`,
      'KanbanService',
      {
        boardId,
        columnId,
  ...
  async moveCard(
    ...
    const { cardId, sourceColumnId, targetColumnId, sourceIndex, targetIndex } =
      moveCardDto;
    ...
    const board = JSON.parse(
      JSON.stringify(this.boards[boardIndex])
    ) as KanbanBoardDto;
    ...
    if (!sourceColumn) {
      this.logger.warning(
        `Column with ID "${sourceColumnId}" not found`,
        'KanbanService',
        {
          boardId,
          sourceColumnId,
    ...
    if (sourceColumnId === targetColumnId) {
      ...
      this.logger.info(
        `Card ${cardId} moved within same column to position ${targetIndex}`,
        'KanbanService',
        {
          boardId,
      ...
      this.logger.info(
        `Card ${cardId} moved from column ${sourceColumnId} to ${targetColumnId} at position ${targetIndex}`,
        'KanbanService',
        {
          boardId,
          cardId,
  ...
  async getBoardsForSocket(): Promise<{
    boards: KanbanBoardDto[];
    storageType: string;
  }> {
  ...
  async updateCard(
    ...
    this.logger.info(
      `Updating card ${cardId} in board ${boardId}`,
      'KanbanService',
      {
        boardId,
        cardId,
  ...
  async deleteCard(boardId: string, cardId: string): Promise<KanbanBoardDto> {
    this.logger.info(
      `Deleting card ${cardId} from board ${boardId}`,
      'KanbanService',
      {
        boardId,
        cardId,
  ...
  // Helper method to convert KanbanBoardData to KanbanBoardDto
  private convertBoardDataToDto(boardData: KanbanBoardData): KanbanBoardDto {
    // Ensure all required properties are present
    return {
      id: boardData.id || boardData._id?.toString() || uuidv4(),
      name: boardData.name || boardData.title || 'Unnamed Board',
      columns: (boardData.columns || []).map((column) => ({
        ...
        phase: column.phase as string,
      })),
      currentPhase: boardData.currentPhase as string,
      phases: boardData.phases as Record<string, any>,
      createdAt: boardData.createdAt as string,
      updatedAt: boardData.updatedAt as string,
    };
  }
}
