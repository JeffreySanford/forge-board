...
import { DiagnosticEvent, DiagnosticTimelineEvent, HealthData, SocketInfo, SocketLogEvent, SocketMetrics } from '@forge-board/shared/api-interfaces';
import { Subscription } from 'rxjs';

// Helper function to create socket responses
function createSocketResponse<T>(event: string, data: T) {
  return {
    event,
    data,
...
export class DiagnosticsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect, OnModuleDestroy {
  ...
  afterInit(server: Server): void {
    ...
    this.timelineSubscription = this.diagnosticsService.timelinePoints$.subscribe(
      (points: DiagnosticTimelineEvent[]) => {
  ...
  /**
   * Emit a diagnostic event to all connected clients
   */
  emitDiagnosticEvent(event: DiagnosticEvent): void {
    this.logger.verbose(`Emitting diagnostic event: ${event.eventType}`);
    const fullEvent = this.diagnosticsService.addEvent(event);
    this.server.emit('diagnostic-event', createSocketResponse('diagnostic-event', fullEvent));
  }
