    ...
    return next.handle(request).pipe(
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.status === 0 || error.status >= 500) {
          ...
          console.error(
            `Critical API Error [${error.status}]: ${error.message}`,
            {
              url: error.url,
          ...
          // Filter out expected documentation 404s that are handled elsewhere
          const isDocumentationRequest =
            request.url.includes('/README.md') ||
            request.url.includes('assets/documentation') ||
