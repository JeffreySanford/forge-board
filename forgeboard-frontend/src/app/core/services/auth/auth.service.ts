...
import { TokenEncryption } from '../../../utils/token.interface';
import { User, UserRole } from '@forge-board/shared/api-interfaces';
import { Environment } from '../../../../environments/environment.interface';

// Define UserRole as an enum if it's only imported as a type
enum UserRoleEnum {
  GUEST = 'GUEST',
  USER = 'USER',
  ADMIN = 'ADMIN'
...
export class AuthService {
   ...
   */
  getToken(): string {
    const encryptedToken = this.envConfig.encryptedJwtToken || '';
  ...
  private loadToken(): void {
    const encryptedToken = (environment as Environment).encryptedJwtToken;
    if (encryptedToken) {
      (environment as Environment).encryptedJwtToken = encryptedToken; // Modifying imported env
    }
  }
  ...
  register(username: string, password: string): Observable<User> {
    return this.http.post<User>(`${this.envConfig.apiUrl}/auth/register`, {
      username, 
      password 
    }).pipe(
      tap(user => {
