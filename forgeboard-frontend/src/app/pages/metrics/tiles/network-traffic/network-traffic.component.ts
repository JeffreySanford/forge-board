...
export class NetworkTrafficComponent
  implements OnInit, OnDestroy, Tile, AfterViewInit
{
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.metricsService.getMetricsStream().subscribe((metrics) => {
        ...
        this.inboundTotal = Math.round(
          this.data.reduce((sum, point) => sum + point.inbound, 0) /
            this.data.length
        );
        this.outboundTotal = Math.round(
          this.data.reduce((sum, point) => sum + point.outbound, 0) /
            this.data.length
        );
  ...
  private updateChart(): void {
    ...
    this.svg
      ...
      .call(
        d3
          .axisBottom(this.x)
          .ticks(5)
          .tickFormat(timeFormatFunction) as unknown as (
    ...
    this.svg
      .select('.axis--y')
      .call(
        d3.axisLeft(this.y).ticks(5) as unknown as (
          selection: d3.Selection<d3.BaseType, unknown, null, undefined>
        ) => void
      );
  }
