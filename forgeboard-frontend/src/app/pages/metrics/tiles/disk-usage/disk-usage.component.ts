...
export class DiskUsageComponent implements OnInit, OnDestroy, AfterViewInit, Tile {
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.metricsService.getMetricsStream().subscribe(metrics => {
        ...
        this.freeSpace = Math.round(this.totalSpace * (1 - this.diskUsage / 100));
  ...
  private updateChart(): void {
    ...
    const paths = this.pieGroup.selectAll('path')
      .data(arcs);
    ...
    // Update center text
    const centerText = this.pieGroup.selectAll('text')
      .data([this.diskUsage]);
