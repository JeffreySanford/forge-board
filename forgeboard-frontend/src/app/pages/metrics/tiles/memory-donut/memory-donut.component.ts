...
export class MemoryDonutComponent implements OnInit, OnDestroy, AfterViewInit, Tile {
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.metricsService.getMetricsStream().subscribe(metrics => {
        ...
        this.memoryUsed = Math.round((this.memoryCurrent / 100) * this.memoryTotal);
        
        // Update formatted values
        this.memoryUsedFormatted = this.formatMemory(this.memoryUsed);
        this.memoryFreeFormatted = this.formatMemory(this.memoryTotal - this.memoryUsed);
  ...
  private initializeChart(): void {
    ...
    this.svg = d3.select(element)
      ...
      .attr('height', height)
      .append('g')
      .attr('transform', `translate(${width / 2},${height / 2})`) as d3.Selection<SVGGElement, unknown, null, undefined>;
      
    // Create arc generator
    this.arc = d3.arc<number, DonutArcData>()
      .innerRadius(radius * 0.6)
      .outerRadius(radius);
      
    // Create pie layout
    this.pie = d3.pie<number>()
      .sort(null)
      .value(d => d);
  ...
  private updateChart(): void {
    ...
    const arcGenerator = (d: DonutArcData): string => this.arc.call(0 as any, d) ?? '';
    const pieGenerator = (vals: number[]): DonutArcData[] => (this.pie.call(0 as any, vals) as DonutArcData[]);

    const data: number[] = [ this.memoryUsed, this.memoryTotal - this.memoryUsed ];
    const arcs = pieGenerator(data);
    const paths = this.donut.selectAll<SVGPathElement, DonutArcData>('path').data(arcs);

    paths.exit().remove();

    paths
      .attr('d', (d: DonutArcData) => arcGenerator(d))
      .attr('fill', (_d: DonutArcData, i: number) => i === 0 ? '#ffe066' : '#ecf0f1');
    ...
    paths.enter()
      ...
      .attrTween('d', function(this: SVGPathElement, d: DonutArcData) {
        const interpolate = d3.interpolate<DonutArcData>({ startAngle: 0, endAngle: 0 } as DonutArcData, d);
