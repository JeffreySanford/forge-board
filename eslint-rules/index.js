  ...
  rules: {
    ...
    'require-non-standalone-components': {
      meta: {
        ...
        docs: {
          description:
            'Enforce non-standalone components by requiring standalone: false',
      ...
      create: function (context) {
        return {
          ...
          "Decorator[expression.callee.name='Component']": function (node) {
            ...
            if (
              decoratorArgument &&
              decoratorArgument.type === 'ObjectExpression'
            ) {
              ...
              if (!standaloneProperty) {
                context.report({
                  ...
                  message:
                    'Component must have standalone: false explicitly set',
                  fix: function (fixer) {
                    // Add standalone: false to the component decorator
                    const lastProp =
                      decoratorArgument.properties[
                        decoratorArgument.properties.length - 1
                      ];
                    const insertionPoint = lastProp
                      ? lastProp.range[1]
                      : decoratorArgument.range[0] + 1;
